...
// get the parameters
  ...
  if (! $error) {
    $hash = password_hash($password, PASSWORD_BCRYPT);//The salt option has been deprecated as of PHP 7.0.0. 
    ...
    $insert = $db->prepare("INSERT INTO users (userId, password_hash, fname, lname, email, expirence, gender, age, address, city, state) 
    VALUES (:id, :hash, :fname, :lname, :email, :expirence, :gender, :age, :address, :city, :state);");
    ...
    $messages[] = "<button><span><a href=\"{$http}/forms/registration.php\" class=\"fill\">Try Again</a></span></button>";
  }
